...
import { usePositions, useScanner, useConfig } from '@/lib/api'
...
export default function DashboardPage() {
  ...
  const totalPnL = openPositions.reduce((sum: number, p: any) => sum + (p.pnl || 0), 0)
  const dailyPnL = riskState?.dailyPnL || 0
  const totalTrades = riskState?.dailyTrades || 0
  const winRate = totalTrades > 0 ? ((totalTrades - (dailyPnL < 0 ? 1 : 0)) / totalTrades) * 100 : 0

  const currentPosition = openPositions.length > 0 ? openPositions[0] : null
  const positionDirection = currentPosition?.direction || 'FLAT'

  const getRiskStatus = () => {
    if (!riskLimit || !riskState) return { color: 'text-gray-400', label: 'UNKNOWN' }
    const dailyLoss = riskState.dailyPnL || 0
    const maxLoss = riskLimit.maxDailyLoss || 1000
    
    if (dailyLoss <= -maxLoss) return { color: 'text-red-500', label: 'LOCKED' }
    if (dailyLoss <= -maxLoss * 0.7) return { color: 'text-yellow-500', label: 'WARNING' }
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          ...
          <Card>
            ...
            <CardContent>
              <div className={`text-2xl font-bold ${riskStatus.color}`}>
                {riskStatus.label}
              </div>
            </CardContent>
          </Card>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold">{positionDirection}</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Size</span>
                    <span className="font-semibold">{currentPosition.quantity} contracts</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Entry Price</span>
                    <span className="font-semibold">{formatCurrency(currentPosition.entryPrice)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Broker</span>
                    <span className="font-semibold uppercase">{currentPosition.broker}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Unrealized P&L</span>
                    <span className={`font-semibold ${getPnLColor(currentPosition.pnl)}`}>
